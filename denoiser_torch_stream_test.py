...
def test_audio_denoising_with_variance(torch_model_path, noisy, frame_length, hidden, depth=4):
    ...
    with torch.no_grad():
        ...
        while frames.shape[1] >= frame_length:
            ...
            print(f'input variance val: {variance[0]:.2f}')
            start_time = time.time()
            # onnx modelinin çalışmsı
            out_frame, next_frame_num, variance_out, next_resample_in, next_resample_out, next_conv_state, next_lstm_state_1, next_lstm_state_2 \
                = streamer.forward(frame, to_numpy(frame_num), to_numpy(variance), resample_input_frame,
                                   resample_out_frame, conv_state, lstm_state_1, lstm_state_2)
            print(f'out variance val: {variance_out[0]:.2f}')
        ...
        if frames.shape[1] > 0:
            ...
            start_time = time.time()
            out_frame, next_frame_num, variance_out, next_resample_in, next_resample_out, next_conv_state, \
            next_lstm_state_1, next_lstm_state_2 = streamer.forward(last_frame, to_numpy(frame_num), to_numpy(variance),
